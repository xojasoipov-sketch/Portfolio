...
import type { ConversationRow, Intent, LeadDraft, MessageRow } from "./types.ts";
...
export async function setConversationIntent(conversationId: string, intent: Intent) {
  await db.from("conversations").update({ intent, updated_at: new Date().toISOString() }).eq("id", conversationId);
}
...
export async function mergeDraft(
  ...
  await db.from("conversations").update({ draft: merged, updated_at: new Date().toISOString() }).eq("id", conversationId);
  return merged;
}

export async function setPendingConfirmation(conversationId: string, pending: boolean) {
  await db.from("conversations").update({ pending_confirmation: pending }).eq("id", conversationId);
}
...
export async function bumpLowConfidence(conversationId: string, low: boolean): Promise<number> {
  const { data } = await db.from("conversations").select("low_confidence_streak").eq("id", conversationId).single();
  const next = low ? (data?.low_confidence_streak ?? 0) + 1 : 0;
  await db.from("conversations").update({ low_confidence_streak: next }).eq("id", conversationId);
  return next;
}
